...
class DrySystemsTablePageState extends State<DrySystemsTablePage> {
  ...
  Future<void> _checkConnectivityAndData() async {
    ...
    if (status != ConnectionStatus.connected) {
      final count = await _dataService.getDrySystemsCount();
      setState(() {
  ...
  Future<void> fetchData({int page = 1, bool forceOnline = false}) async {
    ...
    try {
      final apiResponse = await _dataService.getDrySystems(
        page: page,
  ...
  void _navigateToDetailView(DrySystemData drySystemData) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DrySystemDetailView(drySystemData: drySystemData),
      ),
    );
  }
  ...
  void _showConnectionHelp() {
    ...
    if (_connectionStatus == ConnectionStatus.noNetwork) {
      ...
      message = 'Your device has internet, but the dry system server is unreachable.';
      actions = [
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text(
                    item.form.reportTo.isNotEmpty ? 
                      item.form.reportTo : 
                      item.form.building.isNotEmpty ? 
                        item.form.building : 
                        'Dry System ${index + 1}',
                    style: const TextStyle(fontWeight: FontWeight.w600),
                  ),
